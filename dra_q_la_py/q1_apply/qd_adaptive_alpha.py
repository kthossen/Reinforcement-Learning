#!/usr/bin/env python3
# qd_adaptive_alpha.py
# VaR–DP with uniform α-grid (no j′-mixing), using csv_mdp_loader.

import numpy as np, argparse, json
from csv_mdp_loader import load_all_domains, DOMAINS

def var_dp_qd(P, R, gamma=0.9, T=100, J=4096):
    S, A = R.shape
            ...
            for a in range(A):
                ...
                for j, alpha in enumerate(alphas):
                    ...
                    if tot <= 0.0:
                        ...
                        continue
                    ws = ws / tot
                    # strict-less-than CDF; pick lower α-quantile
                    cum_lt = np.concatenate(([0.0], np.cumsum(ws[:-1])))
                    idx = np.searchsorted(cum_lt, float(alpha), side="right") - 1
                    if idx < 0: idx = 0
                    if idx >= vs.size: idx = vs.size - 1
                    q_curr[s, j, a] = base + gamma * vs[idx]
        q_prev = q_curr
    return q_prev
...
def compute_row(data_dir=".", alpha=0.25, gamma=0.9, T=100, J=4096):
    ...
    for name in DOMAINS:
        ...
        qT = var_dp_qd(P, R, gamma, T, J)
        qlow = q_lower_at_alpha(qT, alpha)
        out[name] = float(qlow[s0].max())
    return out
...
if __name__ == "__main__":
    ...
    paper_print_row("qd", row, args.decimals)
